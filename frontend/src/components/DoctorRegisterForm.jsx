...
function DoctorRegisterForm() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      const res = await API.post("/doctor/register", registrationData);
...
Available Properties: ${Object.keys(res.data || {}).join(', ')}
      ...
      if (isSuccessful) {
        ...
        setSuccess(`Registration successful! Welcome Dr. ${displayName}! Redirecting to login page...`);
        ...
        const timer = setInterval(() => {
          setSuccess(`Registration successful! Welcome Dr. ${displayName}! Redirecting to login page in ${countdown} seconds...`);
          countdown--;
...
Error Data: ${JSON.stringify(err.response.data, null, 2)}
        ...
        if (err.response.status === 400) {
          const errorMessage = err.response.data?.message || err.response.data || "Invalid registration data";
          ...
          const errorMessage = err.response.data?.message || err.response.data || err.response.statusText;
