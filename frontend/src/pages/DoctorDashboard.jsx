...
const fetchPatientsForDoctor = async (doctorId) => {
  try {
    const token = localStorage.getItem("token");
    const response = await fetch(`http://localhost:5011/api/Client/doctor/${doctorId}`, {
      headers: {
...
// Fetch symptoms for a selected patient using username
const fetchSymptomsForPatient = async (patient) => {
  if (!patient?.userName) return;

  try {
    const token = localStorage.getItem("token");
    const response = await fetch(`http://localhost:5011/api/Symptoms/client/username/${patient.userName}`, {
      headers: {
        "Authorization": `Bearer ${token}`,
        "accept": "application/json",
      },
    ...
    const data = await response.json();
    console.log("Fetched symptoms for patient:", data);

    // Save to state
    setSymptoms(data.success ? data.data : []);
  } catch (error) {
...
const handleNextStep = async () => {
  ...
  try {
    // Fetch symptoms using the patient object
    await fetchSymptomsForPatient(selectedPatient);

    // Optionally get prediction automatically after symptoms are loaded
    await getPrediction();
    ...
    alert("Failed to load symptoms or prediction. Please try again.");
  }
};

// Updated getPrediction function to send only selected symptoms
const getPrediction = async () => {
  ...
  try {
    ...
    const sex = selectedPatient.sex || "female"; // adjust if needed
    ...
    const response = await fetch("http://127.0.0.1:8000/predict", {
      ...
      body: JSON.stringify({ symptoms: symptomsList, sex }),
    });
    ...
    alert("Failed to get prediction from the ML model");
  }
};

// Toggle symptom selection when clicking a symptom card
...
<>
  ...
  >
    ...
    >
      ...
      >
        Select a patient to begin symptom analysis and receive predictions from the ML model.
      </p>
    </div>
        ...
        {currentStep === 1 && (
          <>
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              <select
                id="patientSelect"
                value={selectedPatient ? patients.indexOf(selectedPatient) : ''}
            ...
            {selectedPatient && (
              ...
              >
                <h3 style={{ fontWeight: '600', marginBottom: '0.5rem' }}>Patient Details</h3>
                <p>
          ...
          <>
            ...
            {symptoms.length > 0 ? (
              ...
              <p style={{ color: '#9ca3af', fontSize: '0.875rem' }}>
                No symptoms recorded for this patient.
              </p>
            )}
