...
private readonly IHttpClientFactory _httpClientFactory;
    ...
    private async Task<Dictionary<string, double>> GetCancerPrediction(List<string> symptoms, string sex)
{
    var client = _httpClientFactory.CreateClient();
    var payload = new { symptoms = symptoms, sex = sex };
    var response = await client.PostAsJsonAsync(
        "https://cdss-ml-service-epcggmgbgrfwhcg8.southafricanorth-01.azurewebsites.net/predict", 
        payload);
...
public async Task<IActionResult> Create([FromBody] MLPredictionCreateDto request)
{
    try
    {
        ...
        // Get ML prediction from external API
        var predictionResult = await GetCancerPrediction(request.Symptoms, request.Sex);
        var topPrediction = predictionResult.OrderByDescending(x => x.Value).First();

        var prediction = new MLPrediction
        {
            ClientId = request.ClientId,
            PredictedDiagnosis = topPrediction.Key,
            ConfidenceScore = topPrediction.Value,
            Symptoms = string.Join(", ", request.Symptoms), // Convert List<string> to string
